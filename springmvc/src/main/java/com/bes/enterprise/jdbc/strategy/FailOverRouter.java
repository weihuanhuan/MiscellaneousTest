...
//        dataSources.add(new DataSourceHolder(DataSource.class.cast(o), name));
    ...
    public void updateDataSources(Map<Object, Object> targetDataSources) {
        if (targetDataSources == null && targetDataSources.isEmpty()) {
            return;
        }

        List<DataSourceHolder> dataSourceHolderList = new LinkedList<>();
        for (Map.Entry<Object, Object> objectObjectEntry : targetDataSources.entrySet()) {
            dataSourceHolderList.add(
                    new DataSourceHolder((DataSource) objectObjectEntry.getValue(), (String) objectObjectEntry.getKey()));
        }
        updateDataSources(dataSourceHolderList);
    }
